...
def zero_shot(agent, test_env):
    ...
    for episode in range(configs['test_episodes']):
        ...
        while not done:
            ...
            obs_dict_, reward, done, _ = test_env.step(action_dict)
    ...
    fpath = agent.checkpoint_dir + f'/zero_shot{configs["test_stage"]}.csv'
...
def main(args=None):
    rclpy.init()
    env = make_env(configs['test_stage'], configs['max_steps'], configs['lidar'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_stage', type=int, default=6, help='Specify the environment stage: 1, 2, 3, 4')
    ...
    configs['test_stage'] = args.test_stage
    configs['stage'] = 6 # load models from this stage
